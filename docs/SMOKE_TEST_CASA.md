...
> **Status:** checklist abaixo **concluído** (piloto em casa).  
> **Próximo passo:** deploy e smoke em produção — [`GO_LIVE_DIA_0.md`](./GO_LIVE_DIA_0.md).  
...
- [x] Login em `/login`
- [x] Dashboard carrega
- [x] Criar ou abrir um lojista em `/dashboard/partners`
- [x] Aprovar um cadastro delivery em `/dashboard/delivery-registrations`
- [x] Torre de controle: ver riders online (se houver)



## 2) Lojista — portal (navegador no celular ou PC)

- [x] Login → cai em `/minha-loja/pedidos`
- [x] **Configurações**: definir horário (ex.: aberto agora) e salvar
- [x] **Produtos**: criar um produto com preço e foto
- [x] **Copiar** link da vitrine e abrir no celular
- [x] Pedido pago aparece em **Novos** (poll 15s)
- [x] **Aceitar** pedido → vira entrega



## 3) Lojista — app Flutter

- [x] Login como lojista
- [x] Ver pedidos / criar corrida manual
- [x] **Ver vitrine** no menu (copia link)
- [x] Meus Produtos / Promoções / Personalizar abrem



## 4) Entregador (app)

- [x] Login, ficar **online**
- [x] Receber oferta (se FCM local funcionar; senão, poll/realtime)
- [x] Aceitar → navegação Mapbox (precisa token Mapbox local)
- [x] Código da loja → PIN do cliente → concluir
...
- [x] Feed, criar post
- [x] Garagem + banner de manutenção se houver alerta
- [x] Manual (guia iniciantes)
- [x] Atalhos Comunidades / Eventos
...
- [x] Abrir `/loja/<slug>`
- [x] Se loja **fechada** (horário), não consegue pedir
- [x] Se **aberta**: montar carrinho, checkout, PIX
- [x] Após pagar (ou simular webhook em sandbox): status atualiza
- [x] `/pedido/<token>`: timeline + **mapa** do entregador quando em rota
